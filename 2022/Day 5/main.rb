...
def crates
  @values.each_with_object([]) do |value, crates|
    crates << value.chars.each_slice(4).to_a.map(&:join).map do
      _1.scan(/\[(.)\]/).first&.first
    end.reverse
...
silver = instructions.each_with_object(crates) do |instruction, crates|
  number, from, to = parse_instruction(instruction)
  number.times { crates[to - 1] << crates[from - 1].pop }
end.map { _1.last }.join

gold = instructions.each_with_object(crates) do |instruction, crates|
  number, from, to = parse_instruction(instruction)
